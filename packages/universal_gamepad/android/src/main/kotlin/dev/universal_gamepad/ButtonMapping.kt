...
object ButtonMapping {
    ...
    fun axisIndexForMotionAxis(axis: Int): Int? {
        return when (axis) {
            ...
            // Plezy fork: Meta Quest Touch controllers report their thumbstick on
            // RX/RY, not X/Y — verified with getevent on a Quest 3, which emits
            // only ABS_RX/ABS_RY and never ABS_X/ABS_Y. Without these two lines
            // the stick is dropped here and never reaches Dart at all.
            // RX/RY is also what many conventional pads use for the right stick.
            MotionEvent.AXIS_RX -> AXIS_RIGHT_STICK_X
            MotionEvent.AXIS_RY -> AXIS_RIGHT_STICK_Y
            else -> null
        }
    }
    ...
    val STICK_AXES = intArrayOf(
        ...
        MotionEvent.AXIS_Z,
        MotionEvent.AXIS_RZ,
        // Plezy fork: see axisIndexForMotionAxis. Quest thumbsticks live here.
        MotionEvent.AXIS_RX,
        MotionEvent.AXIS_RY,
    )
